...
with open(INPUT_FILE, 'r') as dataRows:
	...
	raceTimeLimits = timeData.split()
	print(f"Time limits:       {raceTimeLimits}")
	...
	finalValue = 1
	for raceNumber in range(0, len(raceTimeLimits)):
		...
		# don't need to actually check charging the value for 0 time or the full time because that results in 0 distance, which has no chance of being greater than another result
		minChargeTime = 1
		maxChargeTime = timeLimit

		waysToWin = 0
		for chargeTime in range(minChargeTime, maxChargeTime):
			moveTime = timeLimit - chargeTime
			distanceCovered = chargeTime * moveTime
			if distanceCovered > distanceRecord:
				waysToWin += 1

		finalValue *= waysToWin
		print(f"Ways to win race #{raceNumber}: {waysToWin}")

	print()
	print(f"Multiplying the ways to win together results in the product: { finalValue }")
	input()